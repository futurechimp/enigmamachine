...
Programmatic requests in Ruby might look something like this:
    ...
    class EnigmaClient
      ...
      def initialize (u, p)
        default_params = basic_auth_creds(u, p)
      end

      def post(path_to_video, encoder_id)
        options = {:body => {:video => {:file => path_to_video}, :encoder_id => encoder_id}}
        self.class.post('/videos', options)
      end

      private

      def basic_auth_creds(username, password)
